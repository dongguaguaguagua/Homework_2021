...
```{r}
library(Seurat) # 如果需要进行PCA
library(ggplot2) # 用于绘图
library(org.Hs.eg.db) # 用于基因符号转换
### read in sample information
# 读取批次信息
batch_info <- read.csv("/home/huzongyao1/HW/HW2/raw_data1/batch.csv")
# 读取TPM文件
tpm_files <- list.files("/home/huzongyao1/HW/HW2/raw_data1/HW2_sf", pattern = "*.sf", full.names = TRUE)
tpm_list <- lapply(tpm_files, function(file) {
    read.table(file, header = TRUE, sep = "\t", row.names = 1)
})
tpm_data <- do.call(cbind, tpm_list) # 合并所有TPM数据
# 将TPM数据转换为基因级别
gene_tpm <- aggregate(tpm_data, by = list(rownames(tpm_data)), FUN = sum)
rownames(gene_tpm) <- gene_tpm[, 1]
gene_tpm <- gene_tpm[, -1] # 去掉第一列（基因名）
# 转换Ensembl ID为基因符号
gene_ids <- sub("\\..*", "", rownames(gene_tpm)) # 去掉版本号，保留基因ID
print(gene_ids)
gene_symbols <- mapIds(org.Hs.eg.db, keys = gene_ids, column = "SYMBOL", keytype = "ENSEMBL", multiVals = "first")
missing_symbols <- gene_ids[is.na(gene_symbols)]
if (length(missing_symbols) > 0) {
    print("未能映射的基因ID:")
    print(missing_symbols)
}
# gene_tpm <- gene_tpm[!is.na(gene_symbols), ]
# rownames(gene_tpm) <- gene_symbols[!is.na(gene_symbols)]
# # 进行PCA分析： 使用log2转换的TPM值进行PCA
# log2_tpm <- log2(gene_tpm + 1)
# pca_result <- prcomp(t(log2_tpm), scale. = TRUE)
# # 可视化PCA结果： 绘制PCA图，并区分组织和批次
# pca_data <- as.data.frame(pca_result$x)
# pca_data$Tissue <- ifelse(grepl("T", rownames(pca_data)), "Tumor", "Normal")
# pca_data$Batch <- batch_info$Batch[match(rownames(pca_data), batch_info$Sample)]

# ggplot(pca_data, aes(x = PC1, y = PC2, shape = Tissue, color = as.factor(Batch))) +
#     geom_point(size = 3) +
#     labs(title = "PCA of Gene Expression Data", x = "PC1", y = "PC2") +
#     theme_minimal()
# # 层次聚类： 你可以使用hclust函数进行层次聚类：
# dist_matrix <- dist(t(log2_tpm))
...
```{r}
### Select those genes that pass variance filtering for batch effect removal


### batch effect removal by Combat
